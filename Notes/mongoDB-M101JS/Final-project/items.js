...
function ItemDAO(database) {
    ...
    this.getCategories = function (callback) {
        ...
        */
        ...
        this.db.collection('item').aggregate([
            ...
            .then((categories) => {
                let total = 0;
                categories.forEach(c => total += c.num);
                var category = {
                    _id: "All",
                    num: total
                };
    ...
    this.getItems = function (category, page, itemsPerPage, callback) {
         ...
         */
        if (category === "All") {
            this.db.collection('item').find().sort({ "_id": 1 }).skip(itemsPerPage * (page)).limit(itemsPerPage).toArray()
                .then((result) => {
                    callback(result);
                })
                .catch((err) => console.log(err));
            ...
            this.db.collection('item').find({ "category": category }).sort({ "_id": 1 }).skip(itemsPerPage * (page)).limit(itemsPerPage).toArray()
                .then((result) => {
                    callback(result);
                })
                .catch(err => console.log(err));
    ...
    this.getNumItems = function (category, callback) {
        "use strict";
        this.db.collection('item').find().count()
    ...
    this.addReview = function (itemId, comment, name, stars, callback) {
        ...
        .then(() => {
            dataB.collection('item').findOne({"_id": itemId})
            .then((result) => {
                callback(result);
            })
            .catch(err => console.log(err));
